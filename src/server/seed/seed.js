...
const seed = async () => {
  ...
  // Profile photo — static public asset (fast, works on Vercel without /api/media)
  const profileImageUrl = '/abdullah.png';

  // Project screenshots load directly from aak-tech.dev CDN (fast, no /api/media round-trip)
  ...
  const experienceCompanies = await Promise.all(
    EXPERIENCE_COMPANIES.map(async (company) => ({
      key: company.key,
      name: company.name,
